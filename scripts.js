...
function formatGender(gender) {
  ...
  // If it starts with a Persian gender, mark as noun
...
async function fetchAndLoadDictionaryData() {
  try {
    ...
    const localResponse = await fetch("persianWords.csv");
    if (!localResponse.ok)
...
async function randomWord() {
  ...
  if (type === "sentences") {
    // Split the Persian and English sentences
...
// Comprehensive Persian inexact-match generator
...
async function search(queryOverride = null) {
  const originalQuery = (
    queryOverride || document.getElementById("search-bar").value
  )
    .toLowerCase()
    .trim()
  ...
  if (type === "stories") {
    ...
    if (!query) {
      ...
      // Filter stories based on the query in both 'titlePersian' and 'titleEnglish'
      matchingResults = storyResults.filter((story) => {
        const persianTitleMatch = story.titlePersian
          .toLowerCase()
          .includes(query);
        const englishTitleMatch = story.titleEnglish
          .toLowerCase()
          .includes(query);
        return persianTitleMatch || englishTitleMatch;
      });
    }
    ...
    if (matchingResults.length === 1) {
      ...
      updateURL(null, type, selectedPOS, null, singleResult.ord); // Set word parameter with the result's Persian term
    ...
    matchingResults = matchingResults.sort((a, b) => {
      ...
      // 1. Prioritize exact match in the Persian or English term
      const isExactMatchA =
        a.ord
          .toLowerCase()
          .split(",")
      ...
      // 2. Prioritize by CEFR level if both English translations or Persian words are identical
      const cefrOrder = { A1: 1, A2: 2, B1: 3, B2: 4, C: 5 };
      ...
      // Check for identical Persian words
...
// Function to toggle the visibility of English sentences and update Persian box styles
function toggleEnglishTranslations(wordId = null) {
  ...
  const persianSentenceDivs = wordId
  ...
  persianSentenceDivs.forEach((div) => {
    ...
    sentences.forEach((sentence) => {
      ...
      if (!uniqueSentences.has(trimmedSentence)) {
        ...
        if (matchedVariation) {
          ...
          const persianPattern = "[\\wčćđšžČĆĐŠŽ]"; // Pattern including Persian letters
          const regex = new RegExp(
            `(${persianPattern}*${matchedVariation}${persianPattern}*)`,
            "gi"
          );
...
function updateURL(query, type, selectedPOS, story = null, word = null) {
  ...
  if (word) {
    ...
    document.title = `${word} - Persian Dictionary`; // Set title to the word
  ...
  if (story) {
    document.title = `${decodeURIComponent(story)} - Persian Story`;
  } else if (query) {
    document.title = `${query} - ${capitalizeType(
      type
    )} Search - Persian Dictionary`;
  } else if (type) {
    document.title = `${capitalizeType(type)} - Persian Dictionary`;
  } else {
    document.title = "Persian Dictionary";
  }
...
function loadStateFromURL() {
  ...
  if (storyTitle) {
    document.title = `${decodeURIComponent(storyTitle)} - Persian Story`;
  ...
  function displayWordIfLoaded() {
    if (results.length > 0) {
      ...
      if (word) {
        ...
        document.title = `${word} - Persian Dictionary`;
        showLandingCard(false);
        resultsContainer.innerHTML = "";
      ...
      if (query) {
        ...
        document.title = "Persian Dictionary | Search in Persian or English";
        clearContainer();
