...
const CSV_URL = "persianStories.csv";
...
async function fetchAndLoadStoryData() {
  ...
  try {
    ...
    storyResults = parsed.map((entry) => ({
      ...entry,
      titlePersian: (entry.titlePersian || "").trim(),
    }));
...
async function displayStoryList(filteredStories = storyResults) {
  ...
  document.title = "Stories - Persian Dictionary";
  ...
  let filtered = filteredStories.filter((story) => {
    ...
    const cefrMatch = selectedCEFR
      ...
      : true;
    const hasPersian = story.persian && story.persian.trim() !== "";
    // NEW: title search across ES + EN titles, like JP
    const matchesSearch =
      !searchText ||
      (story.titlePersian &&
        story.titlePersian.toLowerCase().includes(searchText)) ||
      (story.titleEnglish &&
        story.titleEnglish.toLowerCase().includes(searchText));

    return genreMatch && cefrMatch && hasPersian && matchesSearch;
  });
  ...
  filtered.forEach((story) => {
    ...
    es.textContent = story.titlePersian;

    titleContainer.appendChild(es);

    if (story.titlePersian !== story.titleEnglish) {
    ...
    li.addEventListener("click", () => displayStory(story.titlePersian));
...
async function displayStory(titlePersian) {
  ...
  const selectedStory = storyResults.find(
    (story) => story.titlePersian === titlePersian
  );

  if (!selectedStory) {
    console.error(`No story found with the title: ${titlePersian}`);
    return;
  }

  document.title = selectedStory.titlePersian + " - Persian Dictionary";

  updateURL(null, "story", null, titlePersian); // Update URL with story parameter
...
`;
  ...
  const finalizeContent = (includeAudio = false) => {
    ...
    for (let i = 0; i < persianSentences.length; i++) {
      const persianSentence = persianSentences[i].trim();
      ...
      contentHTML += `
    <div class="couplet">
      <div class="japanese-sentence">${persianSentence}</div>
  ...
  `;
    ...
    if (storyContent) {
      ...
      titleNode.innerHTML = `
  <h2 class="sticky-title-japanese">${selectedStory.titlePersian}</h2>
  ${
    selectedStory.titlePersian !== selectedStory.titleEnglish
...
`;
  ...
  const standardizedPersian = selectedStory.persian.replace(/[“”«»]/g, '"');
  const standardizedEnglish = selectedStory.english.replace(/[“”«»]/g, '"');
  const sentenceRegex =
    /(?:(["]?.+?(?<!\bMr)(?<!\bMrs)(?<!\bMs)(?<!\bDr)(?<!\bProf)(?<!\bJr)(?<!\bSr)(?<!\bSt)(?<!\bMt)[.!?]["]?)(?=\s|$)|(?:\.\.\."))/g;
  let persianSentences = standardizedPersian.match(sentenceRegex) || [
    standardizedPersian,
  ];
  ...
  persianSentences = combineSentences(persianSentences);
