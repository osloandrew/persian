...
// Persian normalization (keep even if you don’t store niqqud)
...
// Split on ASCII hyphen and Persian maqaf (U+05BE), return non-empty parts
...
// Generate candidates by stripping stacked Persian prefixes (up to 3)
...
function candidateVariants(surfaceToken) {
  ...
  for (const b of bases) {
    for (const v of prefixStrips(b)) variants.add(normalizePersian(v));
  }
...
async function renderClozeQuestion(randomWordObj, uniqueDisplayedTranslations) {
  ...
  const baseNorm = normalizePersian(baseWord);
  let clozedForm = null;
  ...
  for (let start = 0; start < tokens.length; start++) {
    for (
      ...
      const normSpace = normalizePersian(joinedSpace);
      const normHyphen = normalizePersian(joinedHyphen);

      if (
        normalizePersian(tokens[start]) === baseNorm ||
...
async function fetchRandomWord() {
  ...
  // Filter out words where the Persian word and its English translation are identical
  filteredResults = filteredResults.filter((r) => {
    // Split and trim the Persian word (handle comma-separated words)
    const persianWord = r.ord.split(",")[0].trim().toLowerCase();
    ...
    // Return true if the Persian and English words are not the same
    return persianWord !== englishTranslation;
  });
